...
void lidarCameraAssociation(const vector<Mat> &image_queue_raw, 
                            const vector<ros::Time> &camera_time_stamp, 
...
void loadBag(const string &filename, vector<Mat>& image_queue, boost::unordered_map<unsigned long, size_t>& stamp_queue, vector<vector<Point3f> >& lidar_queue)
{
    ...
    lidarCameraAssociation(image_queue_raw, camera_time_stamp, lidar_queue_raw, lidar_time_stamp, 
        image_queue, stamp_queue, lidar_queue);

}

/////////////////////////////////////////////////////////////////////////////////////////
// TODO
// replace stamp_queue vector with hash
void lidarCameraAssociation(const vector<Mat> &image_queue_raw, 
                            const vector<ros::Time> &camera_time_stamp, 
                            const vector<vector<Point3f> > &lidar_queue_raw, 
                            const vector<ros::Time> &lidar_time_stamp, 
                            vector<Mat> &image_queue, 
                            vector<unsigned long> &stamp_queue,
                            vector<vector<Point3f> > &lidar_queue){
    size_t n = image_queue_raw.size();
    ros::Time t0 = camera_time_stamp[0];
    for (size_t i=0; i<n; i++){
...
void loadBag(const string &filename, vector<Mat>& image_queue, vector<unsigned long>& stamp_queue, vector<vector<Point3f> >& lidar_queue)
{
    rosbag::Bag bag;
    try{
        bag.open(filename, rosbag::bagmode::Read);
